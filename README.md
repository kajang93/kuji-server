...
### AI 비전 인텔리전스 (예정 ✨)
- **AI 오토 캡셔닝 (Auto Captioning)**: 사용자가 대충 찍은 사진을 업로드해도 Vision AI (GPT-4o, Google Cloud Vision 등)가 이미지를 분석하여 텍스트 설명(상품명, 상태 등)을 자동 작성해주는 기능
- **AI 이미지 인핸스먼트 (Image Decoration)**: Photoroom API, Claid.ai 또는 Stable Diffusion 등을 연동하여 사진의 배경을 자동으로 지우고(누끼), 스튜디오 퀄리티의 깔끔한 배경 및 조명 효과를 합성해주는 상품 사진 보정 자동화
