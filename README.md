...
- **실제 SMS 인증 (알리고)**: 사업자 심사 완료 후, 현재 Mock(가짜) 처리된 SMS 발송 로직을 실제 **알리고(Aligo) API** 키로 교체 및 실서비스 연동 예정
- **소셜 로그인 (OAuth)**: 카카오 로그인은 완료되었으나, **네이버 및 구글 로그인**은 앱 심사 및 API 키 발급 이후 연동 진행 예정
